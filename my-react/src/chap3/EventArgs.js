export default function EventArgs() {
    ...
    return (
        <div>
            <button id="dt" onClick={e => current(e, 'datetime')}>日時を取得</button>
            <button id="date" onClick={e => current(e, 'date')}>日付を取得</button>
            <button id="time" onClick={e => current(e, 'time')}>時刻を取得</button>
        </div>
    )
}